...
export class AddProjects extends React.Component {
  ...
  onSave = () => {
    ...
    const addProjectsJoin = addProjects && addProjects.join || []
    const listJoin = addProjects.joinChild.concat(addProjectsJoin.map(item => item._id));
    ...
    // str G
    const customJoin = arr => (arr ? arr.map(e => e && e.name).join(', ') : '');
