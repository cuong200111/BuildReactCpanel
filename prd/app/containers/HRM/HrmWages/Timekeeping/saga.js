...
export function* fetchTimekeepings(action) {
  try {
    ...
    const response = yield call(request, `${API_TIMEKEEPING}/${tableId ? tableId : 'currentMonth'}?${serialize(rest)}&limit=${limit ? limit : '30'}&skip=${currentPage ? currentPage * limit : '0'}&sort=-updatedAt&`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
