...
function CellEditingModal(props) {
    ...
    () => {
      ...
      if (newState && newState.row && newState.row.hrmEmployeeId) {
        ...
        if (newState.day && (!newState.day.shifts || (newState.day.shifts && !newState.day.shifts.name))) {
          ...
          const { name, data, startTime, endTime } = defaultShift;
          const newData =
            Array.isArray(data) &&
            data.length &&
            data.map(item => ({ title: item.title, startTime: item.startTime, endTime: item.endTime, _id: item._id }));
          // thay đổi startTimeOt thành startTime
          setLocalState({ ...newState, day: { ...newState.day, shifts: { name, data: newData, OTTime: { startTime, endTime } } } });
          // setLocalState({day:{shifts:{checkedOT: true}}})
        }
      }
    },
    [cellData],
  ...
  const handleChangeShifts = useCallback(
    e => {
      ...
      const { name, data, startTime, endTime } = value;
      const newState = {
        ...localState,
        day: {
          ...localState.day,
                      ...
                      <React.Fragment>
                        <Grid item xs={6}>
                          <CustomInputBase
                            ...
                            label="Bắt đầu OT"
                            // value={item.startTime}
                            value={localState.day.shifts.OTTime.startTime}
                        ...
                        <Grid item xs={6}>
                          <CustomInputBase
                            ...
                            label="Kết thúc OT"
                            // value={item.endTime}
                            value={localState.day.shifts.OTTime.endTime}
                            onChange={handleChangeOT}
                          />
                        </Grid>
                      </React.Fragment>
