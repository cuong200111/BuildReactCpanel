...
function TimekeepTable(props) {
  const { symbols, onSymbolChange, onCellClick, day, row, classes, onShow, color = {} } = props;
  ...
  return (
    <>
      ...
      <TableCell
        style={{ backgroundColor: colorCell(), textAlign: 'center', ...color }}
        onClick={handleClick}
        align="center"
        ...
        <>
          {day.verified && (
            <Tooltip
              title="Đã xác nhận"
              style={{
                transform: day.symbol ? 'translate(380%, -50%)' : 'translate(380%, -130%)',
                width: 14,
                height: 14,
                borderRadius: '50%',
                color: 'white',
                backgroundColor: '#2bd02b',
              }}
            >
              <Done />
            </Tooltip>
          ...
          {Array.isArray(dataInOutFinger) && dataInOutFinger.length > 0 ? dataInOutFinger.map((item, index) => {
            ...
            <>
              {Array.isArray(dataInOutFace) && dataInOutFace.length > 0
                ? dataInOutFace.map((item, index) => {
                    if (item.in) {
                      return (
                        <div style={{ width: 75 }}>
                          {item.in || ''} - {item.out || ''}
                ...
                : null}
              {Array.isArray(dataInOutFinger) && dataInOutFinger.length > 0
                ? dataInOutFinger.map((item, index) => {
                    return (
                      <div style={{ width: 75 }}>
                        {item.in} - {item.out}
                      </div>
