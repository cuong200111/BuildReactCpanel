...
function WagesSalaryDetail(props) {
  ...
  const formatData = (sortedFormulaAttributes, newListPageColumns) => {
    ...
    console.log(detailWagesSalary, "dataSource")

    const newDisplayData = detailWagesSalary.map((hrmData, index) => {
      const { dataSource: hrmDataSource , confirmedPaycheck} = hrmData;
      ...
      const defaultCellDataItem = {
        ...
        year: hrmData.year,
        confirmedPaycheck: confirmedPaycheck ? <p style={{color:"green", fontWeight:"bold"}}>Đã xác nhận</p> : <p style={{color:"blue", fontWeight:"bold"}}>Chưa xác nhận</p>
      };
    ...
    console.log(newListPageRows, 'newListPageRows')
  ...
  const arrayProcessing = val => {
    ...
    newColumns.push( {
      checked: true,
      title: 'Xác nhận trên App',
      name: 'confirmedPaycheck',
  ...
  const handleCloseDialogSalary = (isFail) => {
    setOpenSendSalary(false);
    setDisableBtn(false)
  ...
  const handleSendMail = data => {
    // setDisableBtn(true);
    sendEmailWagesSalary(data, handleCloseDialogSalary);
  };
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    sendEmailWagesSalary: (data, handleClose) => dispatch(sendMailWageSalary(data, handleClose)),
