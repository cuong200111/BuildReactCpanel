...
export function* getAllConfigTimekeepingSaga(action) {
  ...
  try {
    ...
    console.log(configTimeKeeping, 'configTimeKeeping')
...
export function* addHolidaySaga(action) {
  ...
  try {
    ...
    if (res && res.status === 1) {
      ...
      yield put(changeSnackbar({ variant: 'error', status: true, message: res.message || 'Thêm mới thất bại' }));
      yield put(addHolidayFailer(res));
    }
  } catch (error) {
    yield put(changeSnackbar({ variant: 'error', status: true, message: error.message ||  'Thêm mới thất bại' }));
...
export function* updateHolidaySaga(action) {
  ...
  try {
    ...
    if (res && res.status === 1) {
      ...
      yield put(changeSnackbar({ variant: 'error', status: true, message: res.message || 'Cập nhật thất bại' }));
      yield put(updateHolidayFailer(res));
    }
  } catch (error) {
    yield put(changeSnackbar({ variant: 'error', status: true, message: error.message || 'Cập nhật thất bại' }));
