...
export class BusinessOpportunitiesReport extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.dialogSelectionRange !== this.state.dialogSelectionRange) {
      const start = `${moment(this.state.dialogSelectionRange.startDate).format('YYYY-MM-DD')}T00:00:00.000Z`;
      const end = `${moment(this.state.dialogSelectionRange.endDate).format('YYYY-MM-DD')}T23:59:00.000Z`;
      const params = {
        filter: {
          createdAt: {
            $gte: start,
      ...
      this.props.onGetItem(params);
      this.props.onGetLogs(params2);
      this.setState({ openPickDate: false, selectionRange: this.state.dialogSelectionRange });
    }
  }
  ...
  render() {
    ...
    const newDataList = arrayData.length > 0 ? arrayData.map(item => dot.dot(item)) : {};
    return (
      <div>
  ...
  handleOpenData = num => {
    ...
    if (Number(num) === 3) {
      this.setState({ openData: true, arrayData: arrayFailed.filter(item => item !== undefined) });
    }
  };
