...
import { changeTabAct } from './actions';
...
import { Dialog as DialogLifetek } from '../../components/LifetekUi';
import HistoryWorkPage from '../HRM/HrmEmployee/HrmEmployeeProfile/HrmEmployeeHistory/HistoryWorkPage';
...
export class AddPersonnel extends React.Component {
    ...
    return (
      <div className={classes.root}>
        ...
        <DialogLifetek title="Tải file" onClose={this.handleCloseDialogScan} open={this.state.openDialogScan}>
          <ScanFileByH onSaveDataScan={this.onSaveDataScan} onCloseDialogScan={this.handleCloseDialogScan} code="hrm" />
        </DialogLifetek>
        {tab === 0 ? (
          <MainInfor
        ...
        {tab === 2 ? (
          <Paper>
            <VerticalTabs style={{ marginLeft: 30 }}>
              ...
              <VerticalTab
                ...
                onClick={() => {
                  this.props.onChangeSnackbar &&
                    this.props.onChangeSnackbar({
                      status: true,
                      message: 'Bạn cần nâng cấp phiên bản để sử dụng tính năng này!',
                      variant: 'warning',
                    });
              ...
              <VerticalTab style={{ textAlign: 'left', textTransform: 'none' }} label="Lịch sử ca làm việc" onClick={this.openHistoryWork} />
            ...
            <SwipeableDrawer
              anchor="right"
              onClose={() => this.props.mergeData({ openHistoryWork: false })}
              open={openHistoryWork}
              width={!miniActive ? window.innerWidth - 260 : window.innerWidth - 80}
            >
              <HistoryWorkPage id={id} onClose={() => this.props.mergeData({ openHistoryWork: false })} />
            </SwipeableDrawer>
          </Paper>
        ) : null}
        {tab === 3 ? (
  ...
  openHistoryWork = openHistoryWork => {
    this.props.mergeData({ openHistoryWork });
  };
  ...
  onSubmit = () => {
    ...
    const id = this.props.id ? this.props.id : this.props.match.params.id;
    console.log(addPersonnel, "addPersonnel")
    const data = {
      ...
      codeShift: addPersonnel.codeShift && addPersonnel.codeShift._id ? [addPersonnel.codeShift._id]  : addPersonnel.codeShift ?  addPersonnel.codeShift.map(item => item._id) : [],
