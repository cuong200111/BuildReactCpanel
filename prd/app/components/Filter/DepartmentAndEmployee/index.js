...
function DepartmentAndEmployee(props) {
  ...
  // useEffect(() => {
  //   if (employeeFilter['organizationUnit.organizationUnitId']) {
  //     // dispatch(getIncreasesOrDecreases(employeeFilter['organizationUnit.organizationUnitId']))
  //   }
  // }, [employeeFilter['organizationUnit.organizationUnitId']])
  ...
  return (
    <>
      <Grid container spacing={isReport ? 0 : 16} style={props.showsortDate && { justifyContent: 'flex-end' }}>
        ...
        {Array.isArray(roleDepartment) && roleDepartment.find(it => it.view == true) || true ? (
          !disableEmployee ? (
            <React.Fragment>
              ...
              {!props.showsortDate && <Grid item md={6} className={isReport && 'reportEmployeeStyle'}>
                <AsyncAutocomplete
