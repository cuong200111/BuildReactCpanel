...
function AdvanceFilterAndListFilterTags(props) {
  ...
  //ket qua khi nhap du lieu vao dialog tim kiem va Nhan button
  const parseQuery = newQuery => {

    const { age, ageFilter, revenue, revenueFilter, lastUsingServiceYear, lastUsingServiceYearFilter, department, employee, provincial, findCustomerWithoutManager, ...rest } = newQuery;
    const newTags = [];
    const parsedQuery = {};
    ...
    if (provincial) {
      parsedQuery.provincial = provincial.value;
      newTags.push({
  ...
  // tim kiem du lieu tu search Goi API voi parseQuery
  const handleSearch = useCallback(newQuery => {
    setQuery(newQuery);
    if (onSearch) {

      const { branches, career, contactWays, customerType, group, introducedWay, productType, provincial, ...rest } = { ...newQuery }
      let newQuerychangekey = {
        ...rest, 'type': customerType, provincial, 'detailInfo.typeCustomer.group': group,
        'detailInfo.typeCustomer.branches': branches, 'detailInfo.typeCustomer.productType': productType,
        'detailInfo.typeCustomer.career': career, 'detailInfo.typeCustomer.contactWays': contactWays,
        'detailInfo.typeCustomer.introducedWay': introducedWay
      }
      onSearch(parseQuery(newQuerychangekey));
    }
  ...
  return (
    <>
      ...
      <AdvanceSearch open={openAdvanceSearch}
        query={query}
        onSave={handleSearch}
