...
function PlanDemo(props) {
  ...
  const { apiUrl, addItem, reload, code, filterItem, profile, id, showTemplate } = props;
  ...
  const [selectKanbans, setSelectKanbans] = React.useState([]);
  ...
  const [openExport, setOpenExport] = React.useState(null)
  ...
  async function reloadData(load = true) {
    ...
    try {
      ...
      if (firstLoad) {
        try {
          const prevSelectionKanbans = JSON.parse(localStorage.getItem("selectKanbans"));
          setSelectKanbans(newColumns.filter(c => prevSelectionKanbans.includes(c.type)));
        } catch (error) {
      ...
      responses.forEach((item, index) => {
        newDataCount[newColumns[index].type] = item.totalCount;
  ...
  const handleDownLoad = async (value) => {
    try {
      ...
      if (selectKanbans && selectKanbans.length) {
        newColumns = newColumns.filter(c => selectKanbans.find(s => s.type === c.type));
      }
      const responses = await Promise.all(
        newColumns.map(c => {
          filter[innerFilterItem] = c.type;
      ...
      if (value === 'Excel') {
        ...
        responses.forEach((item, index) => {
          newDataCount[newColumns[index].type] = item.totalCount;
          const dataNew = item.data.map((innerItem, index) => {
            return {
              "Tên CV": innerItem.name,
              "Mô tả": innerItem.description,
              "Loại công việc": innerItem.categoryName,
              "Người quản lý/giao việc": innerItem.taskManagerStr,
              "Người tham gia/xử lý": innerItem.joinStr,
              "Ngày bắt đầu": moment(innerItem.startDate).format('DD/MM/YYYY'),
              "Ngày Kết thúc": moment(innerItem.endDate).format('DD/MM/YYYY'),
              "Độ ưu tiên": taskPrioty[innerItem.priority - 1]
            }
          })
          const workSheetList = XLSX.utils.json_to_sheet(dataNew);
          XLSX.utils.book_append_sheet(workBook, workSheetList, newColumns[index].name);
        });
        
        const dateExport = newColumns.map((item) => {
          return {
            "Trạng thái CV": item.name,
            "Số lượng": newDataCount[item.type]
          }
        });
  
        const workSheet = XLSX.utils.json_to_sheet(dateExport)      
        XLSX.utils.book_append_sheet(workBook, workSheet, "Thống kê trạng thái công việc");
       
        XLSX.write(workBook, { bookType: "xlsx", type: "binary" })
        XLSX.writeFile(workBook, "thong_ke_trang_thai_cong_viec.xlsx")
        // -----------------export PDF-----------------
      }
      // eslint-disable-next-line no-empty
    } catch (error) {
    ...
    // else if (value === 'PDF') {
    //   const doc = new jsPDF()
    //   const dataHeader = column.map(item => {
    //     return item.title
    //   })
    //   // return [JSON.stringify(item.description), JSON.stringify(item.projectId.name), JSON.stringify(item.progress), JSON.stringify(item.projectId.name), JSON.stringify(item.categoryName), JSON.stringify(item.joinStr)]
    //   const dataBody = data.Retest.map(item => {
    //     return [item.description, item.projectId.name, item.progress, item.projectId.name, item.categoryName, item.joinStr]
    //   })

    //   dataBody.forEach((row) => {
    //     row[2] = row[2].toString(); // convert number to string
    //   });
      ...
      <div className="kanban-main">
        ...
        <Grid container spacing={8} alignItems="center" style={{ padding: '0px 10px' }}>
          ...
          <Grid item style={{ flex: 'auto' }}>
            <Fab onClick={(e) => { setExportAnchor(e.currentTarget) }} color="primary" style={{ marginLeft: 15, width: 40, height: 40, minWidth: 40 }}>
          ...
          <Menu keepMounted open={Boolean(exportAnchor)} onClose={() => { setExportAnchor(null) }} anchorEl={exportAnchor}>
            <MenuItem onClick={() => { handleDownLoad('Excel') }}>Xuất Excel</MenuItem>
            {/* <MenuItem onClick={() => { handleDownLoad('PDF') }}>Xuất PDF</MenuItem> */}
          </Menu>
        ...
        <Grid item style={{ width: 'calc(95%/8)' }}>
          ...
          <Dialog onClose={() => setDialogPlanFilter(false)} onSave={onSaveDialog} saveText="LƯU" open={dialogPlanFilter}>
            ...
            <Grid item style={{ width: '100%', margin: '8px 0' }}>
              <Autocomplete
                onChange={(value) => {
                  localStorage.setItem("selectKanbans", JSON.stringify(value.map(v => v.type)));
                  setSelectKanbans(value);
                }}
                suggestions={columns}
                value={selectKanbans}
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div ref={containerRef} className="kanban-container" style={{ padding: '0px 7px' }}>
            {columns.filter(c => {
              if (selectKanbans && selectKanbans.length) {
                return selectKanbans.find(s => s.type === c.type);
              }
              return true;
            }).map(item => (
              <Column
        ...
        />
        <ExcelTable data={dataNoFormat} columns={columns} dataCount={dataCount} selectKanbans={selectKanbans}/>
      </div>
    </Paper>
  );
}
...
const Column = React.memo(props => {
  const { color, name, data, id, addItem, openDialog, code, reloadData, count, noLoadmore, isReset, allowAdd, allowEdit } = props;
  const [loading, setLoading] = React.useState(false);
    ...
    <div className="kanban-column">
      ...
      {allowAdd ? (
        <div className="main-kanban-column-subtitle">
          <Add onClick={() => addItem(id, code, props)} style={{ cursor: 'pointer', color: '#2196F3', fontSize: '1.5rem', fontWeight: 'bold' }} />
        </div>
      ) : null}
...
const Item = React.memo(({ data, index, openDialog, reloadData, allowEdit, miniActive }) => {
  const [open, setOpen] = React.useState(false);
        ...
        {(provided, snapshot) => (
          ...
          >
            <div className="tasks-kanban-item">
              ...
              <Tooltip title={<span style={{ fontSize: "14px" }}>{data.name}</span>} placement='bottom-start'>
              ...
              <p>
                Loại công việc: <span style={{ color: priotyColor[data.priority - 1] }}>{data.categoryName}</span>
              </p>
