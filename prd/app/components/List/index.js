...
class ListPage extends React.PureComponent {
  ...
  getData = dt => {
    ...
    //fetch Data rely on queryString
    fetch(URL, {
      method: 'GET',
      headers: {
    ...
    return (
      <GridContainer>
        ...
        <MenuAction
          ...
          sendWagesSalary={sendWagesSalary}
    ...
    return (
      ...
      <GridItem md={12} className="CustomlistTable">
        <Grid rows={rows} columns={newCls}>
          ...
          {
            height ? <VirtualTable height={height ? height : 'auto'} columnExtensions={columnExtensions} messages={{ noData: 'Không có dữ liệu' }} /> :
            <VirtualTable  columnExtensions={columnExtensions} messages={{ noData: 'Không có dữ liệu' }} />
          }
...
const MenuAction = React.memo(
  ({
    ...
    const [openDialogTimesheet, setOpenDialogTimesheet] = useState(false)
    ...
    const handleCloseDialogTimesheet = () => {
      setOpenDialogTimesheet(false)
    }
    const handleSendTimesheet = () => {
      // fetch()
      const id = idTimesheet
      let API = sendTimesheet ? API_SEND_TIMESHEET : sendWagesSalary ? API_SEND_WAGESSALARY : '';
      let mesSuccess = sendTimesheet ? 'Gửi bảng chấm công thành công' : sendWagesSalary ? 'Gửi bảng lương thành công' : '';
      let mesError = sendTimesheet ? 'Gửi bảng chấm công thất bại' : sendWagesSalary ? 'Gửi bảng lương thất bại' : '';

      if (id) {
        fetchData(`${API}/${id}`, 'PUT', { confirmApp: true }).then((res) => {
          ...
          if (res.status) {
            setOpenDialogTimesheet(false)
            setSnackbar({ open: true, message: res.message || mesSuccess, variant: 'success' });
          } else {
            setOpenDialogTimesheet(false)
            setSnackbar({ open: true, message: res.message || mesError, variant: 'error' });
          }
        }).catch((err) => {
          //  baso looix
          setOpenDialogTimesheet(false)
          console.log(err, 'data ');
          setSnackbar({ open: true, message: err.message || mesError, variant: 'error' });
        })
      } else {
        setOpenDialogTimesheet(false)
        setSnackbar({ open: true, message: mesError, variant: 'error' });
        //  baso looix
      }
    ...
    function handleTemplate() {
      console.log(6668888, id, code, state.typeFile, tabEx);
      // tabEx === 'process'
      //   ? state.typeFile === 'PDF'
    ...
    return (
      <div>
        <Menu open={Boolean(anchorEl)} onClose={handleClose} anchorEl={anchorEl}>
          ...
          {sendTimesheet && <MenuItem onClick={() => {
            handleClose()
            setIdTimesheet(itemCurrent._id)
            setOpenDialogTimesheet(true)
          }}>Gửi bảng chấm công</MenuItem>}
          {sendWagesSalary && <MenuItem onClick={() => {
            handleClose()
            setIdTimesheet(itemCurrent._id)
            setOpenDialogTimesheet(true)
          }}> Gửi Bảng Lương </MenuItem>}
        ...
        >
          ...
          <AsyncAutocomplete
            ...
            filter={{
              clientId: clientId,
              approveType: 1,
              organizationId: { $in: profile && profile.organizationUnit && profile.organizationUnit.organizationUnitId },
            }}
        ...
        />
        {/* dialog gửi bảng chấm công */}
        <Dialog
          open={openDialogTimesheet}
          onClose={handleCloseDialogTimesheet}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
          fullWidth
        >
          <DialogTitle id="alert-dialog-title">{sendTimesheet ? 'Bạn chắc chắn muốn gửi bảng chấm công không?' : sendWagesSalary ? 'Bạn chắc chắn muốn gửi bảng lương ?' : ''} </DialogTitle>
          <DialogActions>
            <Button variant="outlined" color="primary" onClick={handleSendTimesheet} >
              Đồng ý
            </Button>
            <Button
              variant="outlined"
              color="secondary"
              autoFocus
              onClick={() => setOpenDialogTimesheet(false)}
            >
              HỦY
            </Button>
          </DialogActions>
